...
public class VineSpreader : MonoBehaviour
{
    ...
    //public  List<gameObject> nodes = List<gameObject>();
    ...
    public void Grow()
    {
        ...
        int k = ((nodes.Length) - 1);
            ...
            max_dist = Vector3.Distance(start.transform.position, nodes[k].transform.position);

            for (int i = 0; i < num_vines; i++)
            {
                Vector3 p1 = GetRandomVector3(start.transform.position, (start.transform.position + nodes[k].transform.position) / 2);
                Vector3 p2 = GetRandomVector3((start.transform.position + nodes[k].transform.position) / 2, nodes[k].transform.position);
                ...
                for (int j = 1; j <= SEGMENT_COUNT; j++)
                {
                    ...
                    Vector3 pos = CalculateCubicBezierPoint(t, start.transform.position, p1, p2, nodes[k].transform.position);
